...
class StatusRepository {
  ...
  Stream<List<Status>> getStatusesMap() {
    return firestore.collection('status').snapshots().map((event) {
      List<Status> statuses = [];
      for (var document in event.docs) {
        var status = Status.fromMap(document.data());
        if (status.whoCanSee.contains(auth.currentUser!.uid) && status.uid != auth.currentUser!.uid) {
          statuses.add(status);
        }
      }
      return statuses;
  ...
  Stream<List<Status>> getStatusStream() {
    return firestore.collection('status').snapshots().map((event) {
      List<Status> statuses = [];
  ...
  Stream<List<Status>> getStatusesMapCurrentUser() {
    return firestore.collection('status').snapshots().map((event) {
      List<Status> statuses = [];
      for (var document in event.docs) {
        var status = Status.fromMap(document.data());
  ...
  Stream<List<Status>> getStatusStreamCurrentUser() {
    return firestore.collection('status').snapshots().map((event) {
      List<Status> statuses = [];
  ...
  void uploadStatus({
    ...
    try {
      ...
      for (int i = 0; i < contacts.length; i++) {
        ...
        userDataFirebase.docs.forEach((doc) {
          print(doc.data());
        });
        print('Here asdfsdsr');
